...
// 支払い方法の日本語変換
function getPaymentMethodJapanese(method) {
    switch (method) {
        case 'credit_card':
            return 'クレジットカード決済';
        case 'bank_transfer':
...
// 決済状況の日本語変換
function getPaymentStatusJapanese(status) {
    switch (status) {
        case 'pending':
            return '決済待ち';
        case 'completed':
...
async function addToGoogleSheets(orderData) {
    ...
    try {
        ...
        const values = [[
            ...
            orderData.orderId,
            orderData.orderType === 'first' ? '初回発注' : '2回目以降',
            `${orderData.lastName} ${orderData.firstName}`,
            `${orderData.lastNameKana} ${orderData.firstNameKana}`,
            orderData.salonName,
            orderData.email,
            orderData.phone,
            `${orderData.postalCode} ${orderData.prefecture}${orderData.city}${orderData.address} ${orderData.building || ''}`,
            ...
            orderData.shipping,
            orderData.total,
            getPaymentMethodJapanese(orderData.paymentMethod),
            getPaymentStatusJapanese(orderData.paymentStatus || 'pending')
        ]];
...
// Google Sheetsの決済状況を更新
async function updatePaymentStatusInSheets(orderId, status = 'completed') {
    if (!sheetsAuth || !process.env.GOOGLE_SHEETS_ID) {
        console.log('Google Sheets not configured');
        return { success: false, error: 'Google Sheets not configured' };
    }
    ...
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            if (rows[i][1] === orderId) { // B列（index 1）が注文ID
                // Q列（index 16）の決済状況を更新
                const updateRange = `Q${i + 1}:Q${i + 1}`;
                await sheets.spreadsheets.values.update({
                    spreadsheetId: process.env.GOOGLE_SHEETS_ID,
                    range: updateRange,
                    valueInputOption: 'RAW',
                    resource: {
                        values: [[getPaymentStatusJapanese(status)]]
                    }
                });

                console.log(`✅ Updated payment status for order ${orderId} to ${getPaymentStatusJapanese(status)}`);
                return { success: true };
            }
        }

        console.log(`Order ${orderId} not found in sheets`);
    ...
    try {
        ...
        <table border="1" style="border-collapse: collapse; width: 100%;">
            ...
            <tr>
                ...
                <td style="padding: 8px;"></td>
                <td style="padding: 8px;">${orderData.shipping === 0 ? '無料' : '¥' + orderData.shipping.toLocaleString()}</td>
            </tr>
            <tr style="background-color: #f5f5f5;">
        ...
        <h3>お支払い方法</h3>
        <p>${getPaymentMethodJapanese(orderData.paymentMethod)}</p>
...
async function sendAdminNotificationEmail(orderData) {
    ...
    try {
        ...
        <h3>お客様情報</h3>
        <p><strong>お名前:</strong> ${orderData.lastName} ${orderData.firstName} (${orderData.lastNameKana} ${orderData.firstNameKana})</p>
